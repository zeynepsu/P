...
machine Client 
receives eWriteTransFailed, eWriteTransSuccess;
sends eWriteTransaction, eReadTransaction;
{
	...
	state ConfirmTransaction {
		...
		on eReadTransFailed do { 
				// assert false, "Read Failed after Write!!"; 
		}
		on eReadTransSuccess goto End with (payload: int ) { 
			// assert payload == randomTransaction.val, "Incorrect value returned !!"; 
		}
	}
