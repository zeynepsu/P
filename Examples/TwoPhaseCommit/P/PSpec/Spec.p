...
{
	var receivedLocalCommits: map[Participant, int];
	...
	state WaitForEvents {
		on eMonitor_LocalCommit do (payload: (participant:Participant, transId: int)){
		...
		on eWriteTransSuccess do {
			...
			receivedLocalCommits = default(map[Participant, int]);
		}
