...
namespace P.Program
{
    ...
    public partial class Application : StateImpl
    {
        ...
        static Application()
        {
            Types.Types_boundedasync();
            Events.Events_boundedasync();
            (isSafeMap).Add("Process", false);
            (isSafeMap).Add("Main", false);
            (machineDefMap).Add("Process", "Process");
            (machineDefMap).Add("Main", "Main");
            (createMachineMap).Add("Process", CreateMachine_Process);
            ...
            (_temp).Add("Main", "Main");
            (_temp).Add("Process", "Process");
            (linkMap).Add("Main", _temp);
            _temp = new Dictionary<string, string>();
            (_temp).Add("Process", "Process");
            (linkMap).Add("Process", _temp);
        }
    }
}
